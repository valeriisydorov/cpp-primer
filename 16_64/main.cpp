...
template <> std::size_t vecCount(const std::vector<const char *>& vec, const char* const& n) {
    return std::count(vec.cbegin(), vec.cend(), n);
}
...
int main() {
    ...
    const char *p = "Hello";
    std::vector<const char *> charVec{"Hi", "Hello", "Hey", "Hello", "Hi", "Hi", "Hey"};
    std::cout << vecCount(charVec, p) << '\n';
